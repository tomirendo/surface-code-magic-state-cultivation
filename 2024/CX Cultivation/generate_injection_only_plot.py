...
def report_on_circuit(arg):
    ...
    p = arg['p']
    cache = arg['cache']
    report = cultiv.ErrorEnumerationReport.from_circuit(arg['circuit'], noise=p, max_weight=5, cache=None)
    return {
        **{
            k: v
...
def main():
    ...
    inputs = []
    import os
    for circ_file in os.listdir('circuits'):
        path = f'circuits/{circ_file}'
        circuit = stim.Circuit.from_file(path)
        metadata = dict([i.split('=') for i in circ_file[:-5].split(',')])
        inputs.append({
            'p': float(metadata['p']),
            'circuit' : metadata['c'],
            'd' : metadata['d'],
