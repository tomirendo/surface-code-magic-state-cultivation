...
class VecSim:
    ...
    def do_ch(self, a: Any, b: Any) -> None:
        f = self.state_slicer({a:True, b: False})
        t = self.state_slicer({a:True, b: True})
        self._buffer[f] = self.state[f]
        self.state[f] *= 1/np.sqrt(2)
        self.state[f] += self.state[t]/np.sqrt(2)
        self.state[t] *= -1/np.sqrt(2)
        self.state[t] += self._buffer[f]/np.sqrt(2)
    ...
    def do_csx(self, a: Any, b: Any) -> None:
        self.do_cs(a, b)
        self.do_cx(a, b)
        self.do_t_dag(a)

    def do_cxs(self, a: Any, b: Any) -> None:
        self.do_cx(a, b)
        self.do_cs(a, b)
        self.do_t_dag(a)
 
    def do_cs_dagx(self, a: Any, b: Any) -> None:
        self.do_cs_dag(a, b)
        self.do_cx(a, b)
        self.do_t(a)

    def do_cxs_dag(self, a: Any, b: Any) -> None:
        self.do_cx(a, b)
        self.do_cs_dag(a, b)
        self.do_t(a)

    def do_ct(self, a: Any, b: Any) -> None:
        self.state[self.state_slicer({a: True, b: True})] *= (1 + 1j) / np.sqrt(2)

    def do_ct_dag(self, a: Any, b: Any) -> None:
        self.state[self.state_slicer({a: True, b: True})] *= (1 - 1j) / np.sqrt(2)
    ...
    def do_cswap(self, a: Any, b: Any, c: Any) -> None:
        """Controlled swap gate."""
        ft = self.state_slicer({a: True, b: False, c: True})
        tf = self.state_slicer({a: True, b: True, c: False})
        self._buffer[tf] = self.state[tf]
    ...
    def do_cs_dag(self, a: Any, b: Any) -> None:
        tt = self.state_slicer({a: True, b: True})
        self.state[tt] *= -1j
    ...
    def do_stim_instruction(
        ...
        if inst.name == 'QUBIT_COORDS':
            ...
            if inst.name == 'MRX':
                for q in inst.targets_copy():
                    self.do_rx(q.qubit_value)
        elif inst.name in ['E', 'CORRELATED_ERROR']:
        ...
        elif inst.name == convertion_dict['CH']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 2):
                t1, t2 = ts[k], ts[k + 1]
                if t1.is_measurement_record_target:
                    if out_measurements[t1.value]:
                        self.do_h(t2.qubit_value)
                else:
                    self.do_ch(t1.qubit_value, t2.qubit_value)

        elif inst.name == convertion_dict['CSX']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 2):
                t1, t2 = ts[k], ts[k + 1]
                self.do_csx(t1.qubit_value, t2.qubit_value)

        elif inst.name == convertion_dict['CS_DAGX']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 2):
                t1, t2 = ts[k], ts[k + 1]
                self.do_cs_dagx(t1.qubit_value, t2.qubit_value)
        elif inst.name == convertion_dict['CXS']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 2):
                t1, t2 = ts[k], ts[k + 1]
                self.do_cxs(t1.qubit_value, t2.qubit_value)

        elif inst.name == convertion_dict['CXS_DAG']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 2):
                t1, t2 = ts[k], ts[k + 1]
                self.do_cxs_dag(t1.qubit_value, t2.qubit_value)

        elif inst.name == convertion_dict['CSWAP']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 3):
                t1, t2, t3 = ts[k], ts[k + 1], ts[k + 2]
                if t1.is_measurement_record_target:
                    if out_measurements[t1.value]:
                        self.do_swap(t2.qubit_value, t3.qubit_value)
                else:
                    self.do_cswap(t1.qubit_value, t2.qubit_value, t3.qubit_value)
        
        elif inst.name == convertion_dict['CCZ']:
            ts = inst.targets_copy()
            for k in range(0, len(ts), 3):
                t1, t2, t3 = ts[k], ts[k + 1], ts[k + 2]
                self.do_ccz(t1.qubit_value, t2.qubit_value, t3.qubit_value)

        elif inst.name == convertion_dict['T']:
            for q in inst.targets_copy():
                self.do_t(q.qubit_value)
        elif inst.name == convertion_dict['T_DAG']:
            for q in inst.targets_copy():
                self.do_t_dag(q.qubit_value)
        else:
